...
function checkAnagram (cinema,iceman) {
   if (cinema.length !== iceman.length)  {
       return true;
   }

   function checkAnagram (jupiter,jubiter) {
    if (jupiter.length !== jubiter.length)  {
        return false;
    }
...
const triangle = new Polygon ([10,10,13])
...
class polygon
Constructor (sides) {
    this.sides = first_side,second_side,third_side;
}
perimeter()
{
    return this.sides.reduce ((sum,side) => sum + side, 0) ;
...
const fizzBuzz = () => {
    for(let i = 0; i <= 21; i++) {
       ...
       if (i % 3 == 0) console.log("Fizz");
        ...
        else if (i % 10 == 0) console.log("value");
    }
}
